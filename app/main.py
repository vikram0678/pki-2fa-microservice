...
# # For now, store seed locally in data/seed.txt
# # In Docker, we'll change this to /data/seed.txt using an environment variable
# SEED_FILE_PATH = os.getenv("SEED_FILE_PATH", "data/seed.txt")

# # Make sure data/ folder exists (for local run)
...
#         # 2. Decrypt encrypted seed (base64 -> RSA OAEP SHA-256)
#         hex_seed = decrypt_seed(body.encrypted_seed, private_key)

#         # 3. Save hex seed to file (locally: data/seed.txt)
...
#     except Exception:
#         # If anything fails: return 500 with error message
...
#         # 4. Calculate remaining seconds (0–29)
#         period = 30
#         now = int(time.time())
...
#     try:
#         # 4. Verify with ±1 window (±30 seconds)
